...
public class Football_GoalKeeperHFly : MonoBehaviour
{
	public static Football_GoalKeeperHFly share;

	public static Action<float> EventChangeFlyDistance = delegate { };
	public static Action<bool> EventChangeIsAIControl = delegate { };

	private Football_GoalKeeper _goalKeeper;
	...
	IEnumerator Start()
	{
		_goalKeeper = GetComponent<Football_GoalKeeper>();
		_animator = GetComponent<Animator>();

		setFlyDistance(0.5f);

		Football_Shooter.EventShoot += eventShoot;
		Football_GoalDetermine.EventFinishShoot += finishShoot;
	...
	void calculateFlyingDistance()
	{           // ham nay dung de tinh toa' xem thu mon fai~ bay sang ben trai hay phai~ bao xa, va tinh van toc' quan' tinh' sau do'
		...
		float distanceNear ;
		if (Football_GoalKeeper.instance._direction == 0)
			//distanceNear = Football_GoalKeeperClone.share.nearDistanceRight;
			distanceNear = 0;
		else
			//distanceNear = Football_GoalKeeperClone.share.nearDistanceLeft;
			distanceNear = 0;
		...
		if (_isAIControl)
		{
			...
			if (distanceMagnitude > distanceFar - _deltaDistance * 0.7f)
			{   // khi x cua _balltarget xa hon tam bat distancefar - _deltaDistance * 0.7 cua thu mon thi se can nhac cho thu mon di chuyen,
	...
	void Update()
	{
		...
		if (_isShoot)
		{
			...
			else if (info.IsName("save"))
			{ // || info.IsName("save center")) {
			...
			{       // neu o frame truoc' con dang cuu' bong ma frame nay da ko con thi co' nghia la moi' chuyen thu state save sang post-save
	...
	public void moveAfterSave()
	{       // sau khi bay nguoi` thi thu mon se tiep' tuc tinh tien' theo truc x nhu la quan' tinh' 1 khoang thoi gian nho~
