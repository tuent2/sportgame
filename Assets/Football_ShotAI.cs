...
public class Football_ShotAI : Football_Shot
{
	public static Football_ShotAI instance;

	public static Action<float> EventChangeCurveRange = delegate {};

	private List<Vector3> _ballPath1;
	
	private bool _isShootByAI = false;
	private MyMultiSelection _multiSelection1;
	//public float _remainTimeToNextPointZ;
	private float yVelocityTarget1;
	private float angleZY1;
	private bool _isDown1;
	...
	//private List<Vector3> _debugPath1 = new List<Vector3>();
    ...
    protected virtual void Update()
    {
        ...
        {
            while (_cachedTrans.position.z > _ballPath1[_index].z && _index >=1 )
            {
                --_index; // neu vi tri cua banh da~ vuot wa diem dang xet va khong con diem~ de xet' thi stop
            }

            if ( _cachedTrans.position.z > _ballPath1[_index].z && --_index <0)
            {
                _isShootByAI = false;
            }
            else
            {
                ...
                Vector3 temp = (_ballParent.InverseTransformPoint(_ballPath1[_index]) - _cachedTrans.localPosition).normalized; // convert vi tri diem dang xet ve he toa do cua parent cua trai banh, lay vector chi phuong
				float angleZX = Mathf.Atan2(temp.x, temp.z);			// tinh goc'
                angleZY1 = Mathf.Atan2(temp.y, temp.z);

                float xVelocityTarget = _zVelocity * Mathf.Tan(angleZX);
                ...
                float tempVel = _zVelocity * Mathf.Tan(angleZY1);            // tinh van toc theo truc y

                if (tempVel < yVelocityTarget1)
                ...
                yVelocityTarget1 = tempVel;

                angleZY1 = angleZY1 * Mathf.Rad2Deg;

                //				_ball.velocity = _ballParent.TransformDirection(new Vector3(xVelocityTarget, yVelocityTarget, _zVelocity));
                _remainTimeToNextPointZ = Mathf.Abs((_ballPath1[_index].z - _cachedTrans.position.z)) / _ball.velocity.z;        // thoi gian con lai de trai banh di den diem dang xet voi' van toc' z hien tai
                ...
                speed.y = Mathf.Lerp(speed.y, yVelocityTarget1, _remainTimeToNextPointZ);
    ...
    //public void shoot1()
    //{
    //    shoot1(_shootDirection, _curveLevel, _difficulty);
    //}

    //public void shoot1(Direction shootDirection, float curveLevel, float difficulty)
    //{
    //    //		Debug.Log("AI shoot");
    //    EventShoot();
    ...
    //    _ballPath1 = createPath(shootDirection, curveLevel, difficulty, _cachedTrans.position);
    //    _index = _ballPath1.Count - 1;           // di nguoc tu cuoi' mang~ ve dau` mang~

    //    /*************** Debug ****************/
    //    _multiSelection1.count = _ballPath1.Count;
    //    for (int i = 0; i < _multiSelection1._gos.Count; ++i)
    //    {
    //        Destroy(_multiSelection1._gos[i]);
    //    }
    //    _multiSelection1._gos.Clear();



    //    for (int i = 0; i < _ballPath1.Count; ++i)
    //    {
	...
	private DataShootAI[] dataShootAI1 = new DataShootAI[] {
